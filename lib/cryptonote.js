...
function simpleKeyKDF (key, iterations) {
  ...
  /* Convert the iterations to hex, as it's a uint64, we need to use
     the big integer library and pad the string with leading 0s */
  const hex = BigInteger(iterations).toString(16).padStart(16, '0')
  ...
  const iterationsHex = Buffer.from(hex, 'hex').swap64().toString('hex')
