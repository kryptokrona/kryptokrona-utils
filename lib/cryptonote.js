...
class CryptoNote {
   ...
   * @property {number} [maximumOutputAmount=100000000000] - The maximum amount that a single output can be in atomic units
   * @property {number} [maximumOutputsPerTransaction=90] - the maximum number of outputs per transaction permitted
   * @property {number} [maximumExtraSize=1024] - the maximum amount of extra data that is supported in a transaction
   * @property {boolean} [activateFeePerByteTransactions=true] - activate logic that checks to make sure that a transaction has enough of a fee for the network rules
   * @property {number} [feePerByte=1.953125] - the network fee per byte of the transaction
   * @property {number} [feePerByteChunkSize=256] - the network fee per byte chunking size
  ...
  constructor (config) {
    ...
    if (config && typeof config === 'object') {
      Object.keys(config).forEach(key => {
        /* If the supplied object key is a function, then it is most
           likely a user supplied cryptographic primitive function */
        if (typeof config[key] === 'function') {
          userCryptoFunctions[key] = config[key]
        } else {
          /* Else it's part of our normal configuration */
   ...
   * @returns {CryptoNote.GeneratedTransaction} A newly generated transaction
   */
   ...
   * @param {number} [unlockTime=0] - The unlock time for the transaction
   * @param {Buffer|Object|string} [extraData] - extra data to include in the transaction
   * @returns {Promise<CryptoNote.GeneratedTransaction>} A newly generated transaction
   */
   ...
   * @param {Buffer|Object|string} [extraData] - extra data to include in the transaction
   * @returns {Transaction} A newly generated transaction
   */
  createTransactionStructure (newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, _async, extraData) {
    return createTransaction(
      ...
      this.config.maximumOutputsPerTransaction,
      this.config.maximumExtraSize,
      this.config.activateFeePerByteTransactions,
      this.config.feePerByte,
      this.config.feePerByteChunkSize,
      extraData)
  }
...
function calculateMinimumTxFee (txSize, feePerByte, feePerByteChunkSize) {
  const chunks = Math.ceil(txSize / feePerByteChunkSize)
  return chunks * feePerByteChunkSize * feePerByte
}
...
/* This method performs basic validation of data and creates the basic
   structure of a transaction to prepare for additional steps (ie. generating
   signatures for the transaction */
function createTransactionStructure (newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extra, maximumOutputAmount, maximumOutputsPerTransaction, maximumExtraSize, _async) {
  unlockTime = unlockTime || 0
  randomOutputs = randomOutputs || []
  feeAmount = parseInt(feeAmount)

  if (isNaN(feeAmount)) throw new Error('Invalid fee amount specified')
  ...
  if (transactionOutputs.outputs.length > maximumOutputsPerTransaction) {
    throw new Error('Tried to create a transaction with more outputs than permitted for the number of inputs supplied')
  }

  /* Sending for free (fusion) has some specific rules
     that we need to check for */
  if (feeAmount === 0) {
    if (transactionInputs.length < 12) {
      throw new Error('Sending a [0] fee transaction (fusion) requires a minimum of [' + Config.fusionMinInputCount + '] inputs')
    }
    if ((transactionInputs.length / transactionOutputs.outputs.length) < Config.fusionMinInOutCountRatio) {
      throw new Error('Sending a [0] fee transaction (fusion) requires the correct input:output ratio count be met')
    }
  }
  ...
  if (_async) {
    /* Use Promise.resolve so even if the result isn't a promise, it still
       works */
    return Promise.resolve(transactionOutputs.outputs)
      .then(outputs => { outputs.forEach(output => { tx.outputs.push(output) }) })
      .then(() => { return [tx, transactionInputs] })
  } else {
    transactionOutputs.outputs.forEach(output => { tx.outputs.push(output) })

    return [tx, transactionInputs]
  }
}

/* This method creates a transaction based upon the supplied information, then
   signs the transaction using the supplied keys */
function createTransaction (
  ...
  _async,
  maximumOutputAmount,
  maximumOutputsPerTransaction,
  maximumExtraSize,
  activateFeePerByteTransactions,
  feePerByte,
  feePerByteChunkSize,
  extra
) {
  if (_async) {
    return Promise.resolve(createTransactionStructure(
      newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extra, maximumOutputAmount, maximumOutputsPerTransaction, maximumExtraSize, _async
    ))
      .then(result => {
        const [tx, transactionInputs] = result
        ...
        for (var i = 0; i < transactionInputs.length; i++) {
          ...
          txInput.outputs.forEach(out => { srcKeys.push(out.key) })
        ...
        return Promise.all(sigPromises)
          .then(sigs => { tx.signatures = sigs })
          .then(() => {
            const minimumFee = calculateMinimumTxFee(tx.size, feePerByte, feePerByteChunkSize)
            if (feeAmount && activateFeePerByteTransactions && feeAmount < minimumFee) throw new Error('Transaction fee [' + tx.fee + '] is not enough for network transmission: ' + minimumFee)
          })
          .then(() => { return tx })
      })
  } else {
    const [tx, transactionInputs] = createTransactionStructure(newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extra, maximumOutputAmount, maximumOutputsPerTransaction, maximumExtraSize, _async)
