...
class CryptoNote {
  ...
  createSubWalletFromPrivateSpendKey (privateSpendKey, subWalletIndex, lang, addressPrefix) {
    ...
    /* Next, we're going to do a rudementary KDF operation
       to stretch our private view key to prevent an
       attacker's ability to navigate through the deterministic
       subwallets easily that results in the new spend "seed" */
    const spend = simpleKeyKDF(privateSpendKey, subWalletIndex)

    /* And finish up by generating the spend keys as normal */
    keys.spend = generateKeys(spend)
  ...
  createTransactionStructure (newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, _async, extra) {
    return createTransaction(newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, _async, this.config.maximumOutputAmount, this.config.maximumOutputsPerInput, extra)
  }
...
function createTransaction (newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, _async, maximumOutputAmount, maximumOutputsPerInput, extra) {
...
function simpleKeyKDF (key, iterations) {
  /* This is a very simple implementation of a
     psuedo kdf implementation handling key
     expansions to derive a new key */
  const iterationsHex = toHex(iterations)
  var newKey = key.slice(0)
  for (var i = 0; i < iterations; i++) {
    newKey = cnFastHash(newKey + iterationsHex)
  }
  return scReduce32(newKey)
}
...
function toHex (value) {
  /* Convert the value to hex, as it's a uint64, we need to use
     the big integer library and pad the string with leading 0s */
  const hex = BigInteger(value).toString(16).padStart(16, '0')

  /* The core software requires the use of BE, but our default here
     is actually LE, so read it into a Buffer, swap it to BE, and
     then dump the hex out */
  return Buffer.from(hex, 'hex').swap64().toString('hex')
}
