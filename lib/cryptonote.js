...
function createTransactionStructure (newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extra, maximumOutputAmount, maximumOutputsPerInput, maximumExtraSize, _async) {
...
function createTransaction (
  ...
  if (_async) {
    return Promise.resolve(createTransactionStructure(
      newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extra, maximumOutputAmount, maximumOutputsPerInput, maximumExtraSize, _async
    ))
      .then(result => {
        ...
        return Promise.all(sigPromises)
          ...
          .then(() => {
            if (activateFeePerByteTransactions && feeAmount < calculateMinimumTxFee(tx.size, feePerByte, feePerByteChunkSize)) throw new Error('Transaction fee is not enough for network transmission')
          })
          .then(() => { return tx })
      })
  } else {
    const [tx, transactionInputs] = createTransactionStructure(newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, extra, maximumOutputAmount, maximumOutputsPerInput, maximumExtraSize, _async)
