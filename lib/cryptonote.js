...
function createTransaction (newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, _async, maximumOutputAmount, extra) {
  ...
  if (_async) {
    ...
    for (i = 0; i < transactionInputs.length; i++) {
      ...
      const sigs = generateRingSignature(txPrefixHash, txInput.keyImage, srcKeys, txInput.input.privateEphemeral, txInput.realOutputIndex, _async)
      tx.signatures.push(sigs)
    }
...
function generateRingSignature (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex, _async) {
  if (!isHex64(keyImage)) {
  ...
  if (_async) {
    return Promise.resolve(validSigs)
      .then(valid => {
        if (valid) return signatures
        throw new Error('Could not create ring signatures')
      })
  }
