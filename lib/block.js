...
const Block = function (opts) {
  ...
  Object.defineProperty(this, 'hash', {
    get: function () {
      return getBlockHash(this._toHashingBlob())
    }
  })

  Object.defineProperty(this, 'longHash', {
    get: function () {
      return getBlockPoWHash(this.majorVersion, this._toHashingBlob(true))
    }
  })

  Object.defineProperty(this, 'isValid', {
    get: function () {
      /* this uses the longHash to compute against the difficulty */
    }
  })
}

Block.prototype._toHashingBlob = function (headerOnly) {
  /* This value only matters for majorVersion >= 2 */
  ...
  const transactions = [ this.minerTransaction.hash ]

  this.transactions.forEach(txn => transactions.push(txn))

  const [err, treeHash] = crypto.tree_hash(transactions)

  if (err) throw new Error('Cannot generate transaction tree_hash')

  writer.writeHash(treeHash)
  ...
  if (this.majorVersion >= this.activateParentBlockVersion) {
    ...
    const [parentErr, parentTreeHash] = crypto.tree_hash_from_branch(this.parentBlock.baseTransactionBranch, this.parentBlock.minerTransaction.hash, 0)
    ...
    this.parentBlock.baseTransactionBranch.forEach((branch) => {
      writer.writeHash(branch)
    })

    writer.writeHex(this.parentBlock.minerTransaction.blob)
