...
const Block = function (daemonResponse, mergedMiningBlockVersion) {
  if (!(this instanceof Block)) return new Block(daemonResponse, mergedMiningBlockVersion)
  ...
  Object.defineProperty(this, 'extraNonce', {
    get: function () {
      for (var i = 0; i < this.block.baseTransaction.extra.length; i++) {
        const extra = this.block.baseTransaction.extra[i]

        if (extra.tag === 2) {
          const buf = Buffer.from(extra.nonce, 'hex')
...
Block.prototype.nextBlob = function () {
  for (var i = 0; i < this.block.baseTransaction.extra.length; i++) {
    const extra = this.block.baseTransaction.extra[i]
    ...
    if (extra.tag === 2) {
      ...
      var currentValue = buf.readUInt32BE(0)
      if (currentValue < 4294967295) {
        buf.writeUInt32BE(++currentValue, 0)
      }

      this.block.baseTransaction.extra[i].nonce = buf.toString('hex')
    }
  }
