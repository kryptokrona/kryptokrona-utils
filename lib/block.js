...
class Block {
   ...
   */
  get hash () {
    const blob = toHashingBlob(this)

    if (this.cached.hashBlob && this.cached.hashBlob === blob && this.cached.hash) {
      return this.cached.hash
    }

    this.cached.hashBlob = blob

    this.cached.hash = getBlockHash(this.cached.hashBlob)
   ...
   */
  get longHash () {
    const blob = this.hashingBlob

    if (this.cached.longHashBlob && this.cached.longHashBlob === blob && this.cached.longHash) {
      return this.cached.longHash
    }

    this.cached.longHashBlob = blob

    this.cached.longHash = getBlockPoWHash(this.majorVersion, this.cached.longHashBlob)
  ...
  _fromBlob (blob) {
    ...
    this._nonce = reader.nextUInt32(true)
  ...
  _toBlob () {
    ...
    writer.writeUInt32(this._nonce, true)
...
function toHashingBlob (block, headerOnly) {
  ...
  if (block.majorVersion >= block.activateParentBlockVersion) {
    ...
    writer.writeHash(block.previousBlockHash)
    writer.writeUInt32(block._nonce, true)
  }
