...
Block.prototype._setExtraNonce = function (value) {
  const extraNonce = findExtraNonce(this.minerTransaction.extra)

  const bigInt = new BigInteger(value)

  const newExtraNonce = bigInt.toString(16).toLowerCase().padStart(extraNonce.length, '0')
  ...
  reader.skip(extraNonce.length)

  writer.writeHex(newExtraNonce)
...
function findExtraNonce (blob) {
  ...
  while (reader.unreadBytes !== 0) {
    ...
    switch (tag) {
      ...
      case 2:
        ...
        const extraNonce = reader.nextBytes(nonceLength).toString('hex')
        const integer = new BigInteger(extraNonce, 16)
        return { offset, length: nonceLength * 2, value: integer.toJSValue() }
      case 3:
  ...
  return { offset: reader.currentOffset, length: 2, value: 0 }
}
