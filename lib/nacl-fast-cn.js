// Copyright (c) 2014, Daniel J. Bernstein, Bernard van Gastel, Wesley Janssen, Tanja Lange, Peter Schwabe, Sjaak Smetsers
// Copyright (c) 2014, Dmitry Chestnykh & Devi Mandiri
...
/*
  function cryptoVerify16 (x, xi, y, yi) {
...
// res = s*P
function geScalarmult (P, s) {
  var p = [gf(), gf(), gf(), gf()]
  var upk = [gf(), gf(), gf(), gf()]
  var res = new Uint8Array(32)
  geNeg(P)
  if (unpackneg(upk, P) !== 0) throw new Error('non-0 error on point decode')
  scalarmult(p, upk, s)
  pack(res, p)
  return res
}
...
function geDoubleScalarmultBaseVartime (c, P, r) {
  ...
  var res = new Uint8Array(32)
  geNeg(P)
  if (unpackneg(uP, P) !== 0) throw new Error('non-0 error on point decode')
  scalarmult(cP, uP, c)
  scalarbase(rG, r)
  add(rG, cP)
  pack(res, rG)
...
function geDoubleScalarmultPostcompVartime (r, Pb, c, I) {
  var uPb = [gf(), gf(), gf(), gf()]
  var uI = [gf(), gf(), gf(), gf()]
  var cI = [gf(), gf(), gf(), gf()]
  var rPb = [gf(), gf(), gf(), gf()]
  var res = new Uint8Array(32)
  geNeg(Pb)
  if (unpackneg(uPb, Pb) !== 0) throw new Error('non-0 error on point decode')
  scalarmult(rPb, uPb, r)
  geNeg(I)
  if (unpackneg(uI, I) !== 0) throw new Error('non-0 error on point decode')
  scalarmult(cI, uI, c)
  add(rPb, cI)
  pack(res, rPb)
  return res
...
function geAdd (P, Q) {
  ...
  geNeg(Q)
  if (unpackneg(uP, P) !== 0) throw new Error('non-0 error on point decode')
  if (unpackneg(uQ, Q) !== 0) throw new Error('non-0 error on point decode')
  add(uP, uQ)
  pack(res, uP)
  return res
}
...
nacl.ll = {
  geScalarmultBase: geScalarmultBase,
  geScalarmult: geScalarmult,
  geDoubleScalarmultBaseVartime: geDoubleScalarmultBaseVartime,
  geAdd: geAdd,
  geDoubleScalarmultPostcompVartime: geDoubleScalarmultPostcompVartime
}
