...
function blockToBlob (block, mmBlockVersion) {
  ...
  if (block.majorVersion >= mmBlockVersion) {
    writer.writeHash(block.parentBlock.previousBlockHash, 'hex')
  } else {
    writer.writeHash(block.previousBlockHash, 'hex')
  }
  ...
  block.baseTransaction.inputs.forEach((input) => {
    writer.writeHash(input.type, 'hex')
  ...
  block.baseTransaction.outputs.forEach((output) => {
    ...
    writer.writeHash(output.type, 'hex')
    writer.writeHash(output.key, 'hex')
  })
